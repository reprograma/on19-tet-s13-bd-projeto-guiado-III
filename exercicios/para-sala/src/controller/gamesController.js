...
const deleteGame = async (req, res) => {
  try {
    ...
    if (findGames == null) {
      return res.status(404).json({ message: `Game with ${id} not found` });
    }
    await findGames.remove();
    res
      .status(200)
      .json({ message: `Game with ${id} was succesfully deleted` });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
const findGameByName = async (req, res) => {
  try {
    const findGame = await gamesModel.find({ name: req.query.name }).populate("Console");
