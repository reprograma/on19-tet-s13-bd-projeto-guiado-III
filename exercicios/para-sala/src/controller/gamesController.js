...
const findAllGames = async (req, res) => {
  ...
  //LOGICA DA ROTA GET PARA O PROJETO CASA

  
  const findGameByName = async (req, res) => {
    try {
      const nameOfTheGame = req.query.name;
      const findGame = await gamesModel.find({ name: nameOfTheGame }).populate("games");
      if (findGame == null) {
        res.status(404).json({message: "Não consegui encontrar o jogo, tente novamente."});
      };
      res.status(200).json(findGame);
    } catch (error) {
      res.status(500).json({message: error.message});
